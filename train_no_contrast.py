...
def main():
    ...
    train_loader = data.DataLoader(
        train_dst, batch_size=opts.batch_size, shuffle=True, num_workers=4, collate_fn=dataset_collate)
    val_loader = data.DataLoader(
        val_dst, batch_size=opts.val_batch_size, shuffle=True, num_workers=4, collate_fn=dataset_collate)
    print("Dataset: %s, Train set: %d, Val set: %d" %
          (opts.dataset, len(train_dst), len(val_dst)))

    epoch_step = opts.num_train // opts.batch_size
    epoch_step_val = opts.num_val // opts.batch_size
    ...
    # Set up optimizer
    optimizer = optim.Adam(model_train.parameters(), opts.fr_lr)
    ...
    path = os.getcwd()
    # enhanced__path = os.path.join(path, 'enhanced_pic')
    # os.makedirs(enhanced__path)
    # lowlight__path = os.path.join(path, 'lowlight_pic')
    # os.makedirs(lowlight__path)
    for epoch in range(Init_Epoch, Interval_Epoch):
        ...
        with tqdm(total=epoch_step, desc=f'Epoch {epoch + 1}/{Interval_Epoch}', postfix=dict, mininterval=1) as pbar:
            for iteration, batch in enumerate(train_loader):
                ...
                # lowlight_path = os.path.join(lowlight__path, name)
                # torchvision.utils.save_image(img_lowlight, lowlight_path)

                enhanced_image_1, enhanced_image, A = DCE_net(images)

                # enhanced_path = os.path.join(enhanced__path, name)
                ...
                '''
                loss_cont = 10 * torch.mean(max(L_con(enhanced_image, GT) - L_con(enhanced_image, L) + 0.3,
                                                L_con(L, enhanced_image) - L_con(L, enhanced_image)))
                loss_cont2 = 5 * torch.mean(max(L_const(enhanced_image, GT) - L_const(enhanced_image, L) + 0.04,
                                                L_con(L, enhanced_image) - L_con(L, enhanced_image)))
                '''
        ...
        with tqdm(total=epoch_step_val, desc=f'Epoch {epoch + 1}/{Interval_Epoch}', postfix=dict, mininterval=1) as pbar:
